...
/*********************************************************************************************************
  全局CONSTANTS 常量定义
*********************************************************************************************************/
//extern u8 tcp_client_flag;//tcp连接状态
...
extern STCP_TRANS_DATA tcp_check;
...
void workACT(void)
{
	...
	else
	{
		...
		{
			if(timerIsOverflow(&ReadCardTimer))//超时
			{
				timerStart(&ReadCardTimer, ReadCardTimeInterver);
...
void Time_Update(void)
{
	...
	if(sys_timer1 > 9)//19)
	{
		...
		if (tcp_check.TCP_timer.isStart && (0 == tcp_check.TCP_timer.isOverflow))
		{
			if (tcp_check.TCP_timer.time == 1) 
				tcp_check.TCP_timer.isOverflow = 1;
			else 
				tcp_check.TCP_timer.time--;
		}
	}
}
